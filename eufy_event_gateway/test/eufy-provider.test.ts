...
test("summarizes PPCS failure stages without private transport data", () => {
  ...
  const summary = ppcsStreamLogSummary(device.model, route, {
    ...
    videoCodec: "h264",
    videoNalTypes: [7, 8, 5],
    mediaStartAttempts: 2,
    mediaStopAttempts: 1,
    mediaStopProtocol: "level1-direct",
  ...
  assert.match(summary, /media_start_attempts=2 media_stop_attempts=1 media_stop_protocol=level1-direct/);
